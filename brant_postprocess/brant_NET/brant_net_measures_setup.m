...
for m = 1:2
    for n = 1:numel(prompt{m})
        pox_x = current_pos.x + (m == 2) * 200;
        pos_y = current_pos.y + (n - half_prompt) * 25;
        var_tmp = strrep(prompt{m}{numel(prompt{m}) - n + 1}, ' ', '_');
%         net_options.(var_tmp) = 0;
        h_chb = uicontrol(  'Parent',               hfig_inputdlg,...
                    'String',               prompt{m}{numel(prompt{m}) - n + 1},...
                    'UserData',             '',...
                    'Tag',                  var_tmp,...
                    'Position',             [pox_x, pos_y, 200, 20],...
                    'Style',                'checkbox',...
                    'BackgroundColor',      figColor,...
                    'Value',                net_vals.(var_tmp),...
...
for m = 1:numel(prompt_options)
    var_tmp = strrep(prompt_options{m}, ' ', '_');
%     net_options.(var_tmp) = 0;
    uicontrol(      'Parent',               hfig_inputdlg,...
...
for m = 1
    ...
    h_chb = uicontrol(  'Parent',               h_panel_option,...
                ...
                'Callback',             @sw_opt,...
                'Enable',               state_tmp,...
                'Value',                net_vals.(var_tmp).(tag_tmp));
	brant_resize_ui(h_chb);
end
