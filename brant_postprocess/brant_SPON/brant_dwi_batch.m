...
for m = 1:num_subj
    ...
    cd(data_pth{m}); % could save a lot characters in script
    
% %     filenames with full path
%     eddy_out = fullfile(data_pth{m}, eddy_out_fn);
%     DWI_brain_out = fullfile(data_pth{m}, DWI_brain_out_fn);
%     DWI_mask_out = fullfile(data_pth{m}, DWI_mask_out_fn);
%     DTI_recon_out = fullfile(data_pth{m}, DTI_recon_out_fn);
%     DTI_tensor_out = fullfile(data_pth{m}, DTI_tensor_out_fn);
%     DTI_FA_out = fullfile(data_pth{m}, DTI_FA_out_fn);
%     DTI_trk_out = fullfile(data_pth{m}, DTI_trk_out_fn);
    ...
    if recon_dti_ind == 1
        ...
        system(sprintf('"%s" -d "%s" -b "%s" -g "%s" -m "%s" -o "%s" -tensor 1 -eig 1',...
                       fullfile(dk_dir, 'bndti_estimate'), eddy_out_fn, bvals_fn{m}, bvecs_fn{m}, DWI_mask_out_fn, DTI_recon_out_fn));
    end
    ...
    if hardi_odf_ind == 1
        ...
        system(sprintf('"%s" -d "%s" -b "%s" -g "%s" -m "%s" -o "%s" -outGFA 1 -tau 0.02533 -sh 4 -ra 1 -lambda_sh 0 -lambda_ra 0 -rdis 0.015',...
                       fullfile(dk_dir, 'bnhardi_ODF_estimate'), eddy_out_fn, bvals_fn{m}, bvecs_fn{m}, DWI_mask_out_fn, HARDI_ODF_out_fn));
    end
    ...
    if hardi_fod_ind == 1
        ...
        system(sprintf('"%s" -d "%s" -b "%s" -g "%s" -m "%s" -o "%s" -outFA 1 -lmax 8 -fa [0.75,0.95] -nIter 50 -lambda 1 -tau 0.1 -hr 300',...
                       fullfile(dk_dir, 'bnhardi_FOD_estimate'), eddy_out_fn, bvals_fn{m}, bvecs_fn{m}, DWI_mask_out_fn, HARDI_FOD_out_fn));
    end
end
