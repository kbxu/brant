function varargout=brant_spm_conman(varargin)
...
if (nargin==0) || ~ischar(varargin{1})
    ...
    [F,cF] = brant_spm_conman('Initialise','on',SPM,STATmode,n,Prompt,Mcstr,OK2chg);
    ...
    brant_spm_conman('D_Setup_CB');
    set(findobj(F,'Tag','D_name'),'String', 'name_tmp');
    set(findobj(F,'Tag','D_ConMtx'),'String', '1 -1');
    
    set(findobj(F,'Tag','D_TF','UserData','T'), 'Value', 1);
    set(findobj(F,'Tag','D_TF','UserData','F'), 'Value', 0);
    
    brant_spm_conman('d_conmtx_cb');
    brant_spm_conman('d_ok_cb');
    brant_spm_conman('done_cb');
    ...
    brant_spm_conman('Initialise','off');
...
switch lower(varargin{1})
    ...
    case 'initialise'
    %======================================================================
    % [F,cF] = brant_spm_conman('Initialise',Vis,SPM,STATmode,n,Prompt,Mcstr,OK2chg)
    ...
    switch lower(Vis)
        ...
        %------------------------------------------------------------------
            ...
            if isempty(F)                         %-Create ConMan win
                [F,H] = brant_spm_conman('CreateFig');
            else                        %-Get handles
            ...
            STAT = brant_spm_conman('TFA',F,'',STATmode);     %-Init. TFA buttons
            set(H.hPrompt,'String',Prompt,'UserData',n) %-Set prompt
            brant_spm_conman('ImDesMtx',xX,H.hDesMtxAx)       %-Depict DesMtx
            brant_spm_conman('ImParEst',xX,H.hParEstAx)       %-Parameter estimability
            brant_spm_conman('ListCon',H.hConList,xCon,STAT,[]) %-List contrasts
            if OK2chg, tmp='on'; else tmp='off'; end    %-OK to change xCon?
            set(H.hNew,'Enable',tmp)                    %-En/dis-able change UI
            %-****  if isempty(xCon), brant_spm_conman(); end %-Go straight to DNewUI
    ...
    case 'imdesmtx'
    %======================================================================
        % brant_spm_conman('ImDesMtx',xX,h)
    ...
    case 'imparest'
    %======================================================================
        % brant_spm_conman('ImParEst',xX,h)
    ...
    case 'listcon'
    %======================================================================
        % brant_spm_conman('ListCon',hConList,xCon,STAT,I)
        ...
        brant_spm_conman('GraphCons',xCon,Q(q),get(hConList,'Parent'))
        brant_spm_conman('StatusLine',get(hConList,'Parent'))


    %======================================================================
    case 'graphcons'
    %======================================================================
        % brant_spm_conman('GraphCons',xCon,I,F)
    ...
    case 'statusline'
    %======================================================================
        % brant_spm_conman('StatusLine',F,str,col)
...
%         F = gcbf;
        F = findobj(0, 'Name', 'SPM contrast manager');
    ...
    %======================================================================
        ...
        if strcmp(get(F,'SelectionType'),'open'), brant_spm_conman('Done_CB'), end


    %======================================================================
    case {'tfa','d_tf'}
    %======================================================================
        % STAT = brant_spm_conman('TFA')
        % STAT = brant_spm_conman('TFA',F,STAT)
        % STAT = brant_spm_conman('TFA',F,STAT,STATmode)
        ...
        %------------------------------------------------------------------
            ...
            if nargin<3
                SPM      = get(F,'UserData');
                xCon     = SPM.xCon;
                brant_spm_conman('ListCon',findobj(F,'Tag','ConList'),xCon,STAT)
            end
        ...
        %------------------------------------------------------------------
            ...
            brant_spm_conman('GraphCons',[],[],F)                     %-Clear contrast plot
            brant_spm_conman('D_Status',F)                            %-Set StatusLine
    ...
    %======================================================================
        % [c,I,emsg,imsg,msg] = brant_spm_conman('ParseCon',cstr,X,STAT)
    ...
    %======================================================================
        % [iX0,I,emsg,imsg] = brant_spm_conman('ParseIStr',str,max)
    ...
    %======================================================================
        ...
        brant_spm_conman('ListCon',hConList,xCon,STAT,[])
...
%         F        = gcbf;
        ...
        brant_spm_conman('D_TF',F,STAT,STATmode);       %-Setup rest of define UI
    ...
    %======================================================================
        ...
        %------------------------------------------------------------------
            ...
            [c,I,emsg,imsg] = brant_spm_conman('ParseCon',str,xX.xKXs,STAT);
            if all(I)
        ...
        %------------------------------------------------------------------
            ...
            [iX0,I,emsg,imsg] = brant_spm_conman('ParseIStr',str,nPar);
        ...
        if all(I)
            ...
            brant_spm_conman('GraphCons',DxCon,-1,F)      %-Depict contrast
        else
...
%         F = gcbf;
        ...
        brant_spm_conman('TFA',F,xCon(end).STAT);                     %-Set STAT
        brant_spm_conman('ListCon',hConList,xCon,xCon(end).STAT,I)    %-ListCon

        %-Hide the DefineNew UI
        %------------------------------------------------------------------
        set(get(findobj(F,'Tag','DefineNew'),'UserData'),'Visible','off')
        set(F,'UIContextMenu',findobj(gcbf,'Tag','ConMan_ConMen'))

        
    %======================================================================
    case 'd_status'
    %======================================================================
        % brant_spm_conman('D_Status',F)
    ...
    %======================================================================
        ...
        set([hT,hF,hA],'CallBack','brant_spm_conman(''TFA'');',...
        ...
        uimenu(hh,'Label','Rename selected contrast...',...
            'Tag','CM_Ren',...
            'CallBack', 'brant_spm_conman(''Rename_CB'')',...
            'Interruptible','off','BusyAction','Cancel');
        set(hh,'CallBack','brant_spm_conman(''FConMenu_CB'')',...
            'Interruptible','off','BusyAction','Cancel');
        ...
        hConList = uicontrol(F,'Style','ListBox','Tag','ConList',...
            ...
            'Max',2,...
            'CallBack','brant_spm_conman(''ConList_CB'')',... 
            'UIContextMenu',hh,...
            'Interruptible','off','BusyAction','Queue',...
            'BackgroundColor','w',...
        ...
        hNew = uicontrol(F,'Style','Pushbutton','String','Define new contrast...',...
            ...
            'ForegroundColor','b',...
            'Callback','brant_spm_conman(''D_Setup_CB'')',...
            'Enable','on',...
        ...
        uicontrol(F,'Style','Pushbutton','String','Reset',...
            ...
            'ForegroundColor','r',...
            'Callback','brant_spm_conman(''Reset_CB'')',...
        ...
        uicontrol(F,'Style','Pushbutton','String','Done',...
            ...
            'ForegroundColor','m',...
            'Tag','Done','UserData',1,...
            'Callback','brant_spm_conman(''Done_CB'')',...
            'Interruptible','off','BusyAction','Cancel',...
        ...
        uimenu(h,'Label','help','Separator','on',...
            'CallBack','spm_help(''brant_spm_conman'')',...
            'Interruptible','off','BusyAction','Cancel');
        uimenu(h,'Label','crash out','Separator','on',...
            'CallBack','brant_spm_conman(''Initialise'',''reset'');',...
            'Interruptible','off','BusyAction','Cancel');
        set(h,'CallBack','brant_spm_conman(''FConMenu_CB'')',...
            'Interruptible','off','BusyAction','Cancel');
        ...
        h = uicontrol(F,'Style','Edit','Tag','D_name',...
            ...
            'Callback','brant_spm_conman(''D_Status'')','Interruptible','off',...
        ...
        set([hDT,hDF],'CallBack','brant_spm_conman(''D_TF'');',...
            'Interruptible','off','BusyAction','Queue')
        H = [H,hDT,hDF];
        ...
        h = uicontrol(F,'Style','Edit','Tag','D_ConMtx',...
            ...
            'BackgroundColor',COLOUR,...
            'Max',2,...
            'CallBack','brant_spm_conman(''D_ConMtx_CB'')',...
            'Interruptible','off','BusyAction','Queue',...
            'UserData',[],...
        ...
        h = uicontrol(F,'Style','Edit','Tag','D_X1cols',...
            ...
            'HorizontalAlignment','Left',...
            'BackgroundColor',COLOUR,...
            'CallBack','brant_spm_conman(''D_X1cols_CB'')',...
            'Interruptible','off','BusyAction','Queue',...
        ...
        h = uicontrol(F,'Style','Pushbutton','String','Reset',...
            ...
            'ForegroundColor','b',...
            'Callback','brant_spm_conman(''D_Reset_CB'')',...
            'Interruptible','off','BusyAction','Cancel',...
        ...
        h = uicontrol(F,'Style','Pushbutton','String','Cancel',...
            ...
            'ForegroundColor','r',...
            'Callback','brant_spm_conman(''D_Cancel_CB'')',...
            'Interruptible','off','BusyAction','Cancel',...
        ...
        h = uicontrol(F,'Style','Pushbutton','String','OK',...
            ...
            'ForegroundColor','m',...
            'Callback','brant_spm_conman(''D_OK_CB'')',...
            'Interruptible','off','BusyAction','Cancel',...
