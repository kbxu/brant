function out = brant_spm_run_realign_estwrite(outdir, varargin)
...
for i=1:numel(job.data)
    ...
    out.sess(i).rpfile{1} =  fullfile(destdir, sprintf('rp_%s.txt', nam));
end;
    ...
    case 1,
        for k=1:numel(job.data)
            ...
            for i=1:length(out.sess(k).rfiles),
                ...
                out.sess(k).rfiles{i} = fullfile(destdir,[job.roptions.prefix, nam, ext, num]);
            end;
        end;
    otherwise,
        for k=1:numel(job.data)
...
if job.roptions.which(2),
    ...
    out.rmean{1} = fullfile(destdir,['mean', nam, ext, num]);
end;
return;
