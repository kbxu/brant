...
function [img,hdr] = load_untouch_nii_img_mod(hdr,filetype,fileprefix,machine,img_idx,dim5_idx,dim6_idx,dim7_idx,old_RGB,slice_idx)
...
if isempty(slice_idx)
    [img,hdr] = read_image_vol(hdr,filetype,fileprefix,machine,img_idx,dim5_idx,dim6_idx,dim7_idx,old_RGB);
else
...
hdr.dime.dim([false, hdr.dime.dim(2:end) < 1]) = 1;
...
if hdr.dime.datatype == 1 || isequal(hdr.dime.dim(4:8),ones(1,5)) || ...
    ...
    img = foptgz(fileprefix, 'rb', {sprintf('%s',precision)}, {uint32(img_siz)}, uint32(off_data));
...
%         end
        img = zeros(img_siz, length(slice_idx)*length(img_idx)*length(dim5_idx)*length(dim6_idx)*length(dim7_idx)); % , precision_mat
        currentIndex = 1;
    else
        img = [];
    end; %if(roman)
...
%                         end
                        ...
                        % ROMAN: begin
                        if(roman)
                            tmp = foptgz(fileprefix, 'rb', {sprintf('%s',precision)}, {uint32(img_siz)}, uint32(pos + off_data));
                            img(:,currentIndex) = tmp{1};
...
% get from load_nii_img_mod
function [img,hdr] = read_image_vol(hdr,filetype,fileprefix,machine,img_idx,dim5_idx,dim6_idx,dim7_idx,old_RGB)
...
%         precision_mat = precision;
%     end
    img = zeros(img_siz, length(img_idx)*length(dim5_idx)*length(dim6_idx)*length(dim7_idx)); % , precision_mat
    currentIndex = 1;
