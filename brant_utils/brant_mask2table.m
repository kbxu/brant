...
% template_info_tmp = importdata(roi_info_fn, '\n');
% rois_str = cellfun(@(x) regexp(x, '[\s]+', 'split'), template_info_tmp, 'UniformOutput', false);

template_nii = load_nii(template_in);
template_nii_mask = template_nii.img > 0.5;

mask_in_nii = load_nii(mask_in);
XYZ = brant_get_XYZ(mask_in_nii.hdr);
...
mask_size_str = num2cell(mask_size);
mask_vals_str = num2cell(mask_vals); % for temp
