...
maskfn = jobman.input_nifti.mask(1);
if ~isempty(maskfn{1})
    if strcmpi(maskfn{1}(end-2:end), '.gz')
        maskfn = gunzip(maskfn{1}, tempdir);
    end
end
...
if isempty(h_chk_fig)
    ...
    set(h_chk_fig, 'KeyReleaseFcn', {@brant_spm_figure_KeyFun, [curr_set, curr_tp, num_subj, curr_tps], subj_ids, nifti_list, maskfn, color_str});
end
