function G = randomizeGraph_kc(G, T, gType)
%randomizeGraph_kc was used to randomizing graph's connected matrix,and
%   keep the connectivity of graph.  
% Inputs:
...
% Usage:
%   G = randomizeGraph_kc(G, T, gType) return a new adjacency matrix having
%   the same degree sequence. 
% Example:
%	G  = CCM_TestGraph1('nograph');
%   newGraph = randomizeGraph_kc(G, 100, 'binary');
% Note:
%   If i1--j1 and i2--j2, will modify to i1--j2,i2--j1, and keep the
%   connectivity of i1->j1 or i2->j2.
...
if(~isconnected(G)),  error('The input graph isn''t conneted!');   end
      	...
      	if((G(i1,j2) == 0) && (G(i2,j1) == 0)),
        	% i1-j1 ==> i1-j2
         	...
         	G(i2,j1) = G(i2,j2);   G(i2,j2) = 0;
          	...
          	if(isconnected(G,i1,j1) && isconnected(G,i2,j2)),
            	J(edge1) = j2;    J(edge2) = j1;     num = num + 1;
            else
                G = bu_G;
           	end
        end
